...
export class FilesystemTauri implements FilesystemPlugin {
  exists = withFolderCheck(
    async (options: ExistsOptions): Promise<ExistsResult> => {
      return {
        exists: await exists(POKEBOOK_DIR + options.path, {
          baseDir: BaseDirectory.Home,
        }),
      };
    }
  );
  writeFile = withFolderCheck(
    async (options: WriteFileOptions): Promise<WriteFileResult> => {
      await writeTextFile(POKEBOOK_DIR + options.path, options.data, {
        baseDir: BaseDirectory.Home,
      });
  ...
  readFile = withFolderCheck(
    async (options: ReadFileOptions): Promise<ReadFileResult> => {
      return {
        data: await readTextFile(POKEBOOK_DIR + options.path, {
          baseDir: BaseDirectory.Home,
        }),
      };
    }
  );
  deleteFile = withFolderCheck(
    async (options: DeleteFileOptions): Promise<void> => {
      await remove(POKEBOOK_DIR + options.path, {
        baseDir: BaseDirectory.Home,
      });
    }
  );
  readDir = withFolderCheck(
    async (options: ReadDirOptions): Promise<ReadDirResult> => {
      const entries = await readDir(POKEBOOK_DIR + options.path, {
        baseDir: BaseDirectory.Home,
      });
