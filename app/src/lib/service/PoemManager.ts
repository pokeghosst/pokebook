...
class PoemManager {
	...
	constructor(db: DatabasePlugin) {
		this.db = db;
	}

	public async save(poem: Poem): Promise<string> {
		const snippet = this.sliceSnippet(poem.text);
	...
	public async list(): Promise<PoemListItem[]> {
...
// TODO: For later - determine the appropriate database plugin based on user's preferences
export const poemManager = new PoemManager(new DatabaseWeb());
